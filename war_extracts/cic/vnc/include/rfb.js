...
that.sendAltTab = function() {
    if (rfb_state !== "normal" || conf.view_only) { return false; }
    Util.Info("Sending Alt-Tab");
    var arr = [];
    arr = arr.concat(keyEvent(0xFFE9, 1)); // Alt
    arr = arr.concat(keyEvent(0xff09, 1)); // TAB
    ws.send(arr);
    
    setTimeout(function(){
    	var arr = [];
    	arr = arr.concat(keyEvent(0xFFE9, 0)); // Alt
    	arr = arr.concat(keyEvent(0xff09, 0)); // TAB
    	ws.send(arr);
    },3000);
   
};

that.sendCtrlSpace = function() {
    if (rfb_state !== "normal" || conf.view_only) { return false; }
    Util.Info("Sending Ctrl-Space");
    var arr = [];
    arr = arr.concat(keyEvent(0xFFE3, 1)); // Control
    arr = arr.concat(keyEvent(0x0020, 1)); // SPACE
    arr = arr.concat(keyEvent(0xFFE3, 0)); // Control
    arr = arr.concat(keyEvent(0x0020, 0)); // SPACE
    ws.send(arr);
};
