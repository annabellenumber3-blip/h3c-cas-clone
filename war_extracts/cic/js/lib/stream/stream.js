			...
			"undefined" != e && "undefined" != c
				&& _array.push("object" === typeof json
							...
							: "[object Function]" === Object.prototype.toString.call(json)
								? encodeURIComponent(e) + "=" + encodeURIComponent(encodeURIComponent(json()))
								: encodeURIComponent(e) + "=" + encodeURIComponent(encodeURIComponent(json)))
		};
		if (!c && url)
...
//	}
    ...
    function falertMessage(msg,warning){
    	var s = '<div style="margin-left:0px;margin-right:0px;margin-top:0px;padding:10px;margin-bottom:10px" class="alert ';
				...
				case "load":
					...
					try {
						if (xhr.readyState == 4 && (xhr.status == 200 || xhr.status == 308)) {
							...
							uploaded = (respJson = responseText) ? respJson.start : -1;
	...
	function Main(cfg){
		...
		this.config = {
			...
			//nameRegexExclude : cfg.nameRegexExclude || false,
			onNameRegexMismatch:cfg.onNameRegexMismatch,
			checkFileName : cfg.checkFileName == null ? true : !!cfg.checkFileName //对文件名检测是否允许输入特殊字符，默认为true
		};
		Parent.apply(this, arguments);
	...
	Main.prototype = {
		...
		upload : function(index) {
			...
			//解决删除上传文件后再次上传后无法上传的问题
			if(index == null) return;
			if (this.uploadInfo[index]) {
			...
			xhr.onreadystatechange = function() {
				...
				try {
					...
					if (token) {
						...
						var msg;
						if(responseText.success == false && responseText.message) {
							msg = responseText.message;
						} else {
							msg = "\u521B\u5EFA\u4E0A\u4F20\u4EFB\u52A1\u5931\u8D25[tokenURL=" + self.get("tokenURL") + "],\u72B6\u6001\u7801:" + xhr.status;
						}
		...
		validateFile : function(uploader) {
			...
			//add by sunxuejiao 2015-3-6
			if(valid){
//				var regex = !1,pattern =/[\u4E00-\u9FA5]/i;
//					name.match(pattern) && (regex = !0);
//				if(regex){
//					this.get("onNameRegexMismatch") ? this.get("onNameRegexMismatch")(info) : this.onNameRegexMismatch(info);
//					valid = !1;
//				}
				if (this.get("checkFileName") == true) {
					var regex = !1,pattern =/^[\A-Za-z0-9\-\_\.]*$/;
					name.match(pattern) && (regex = !0);
					if(!regex){
						this.get("onNameRegexMismatch") ? this.get("onNameRegexMismatch")(info) : this.onNameRegexMismatch(info);
						valid = !1;
	...
	window.isCanBeHtml5 = bStreaming;
