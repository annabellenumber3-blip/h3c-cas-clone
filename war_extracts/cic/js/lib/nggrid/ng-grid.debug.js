...
angular.module('ngGrid.services').factory('$sortService', ['$parse', '$utilityService', function($parse, $utils) {
    ...
    sortService.guessSortFn = function(item) {
        ...
        switch (itemType) {
            ...
            case "string":
                ...
                // Issue 201603220243: if ip String return ip String sort. f10574 20160322
            	var regExp = new RegExp("/^[-+]?[£$¤]?[\d,.]+%?$/");
                return regExp.test(item) ? (sortService.isIp ? sortService.sortIpStr : sortService.sortNumberStr) : sortService.sortAlpha;
            default:
    ...
    //check whether item is ip or not
    sortService.isIp = function(item) {
    	var regExp = new RegExp("/^([1-9]\d?|1\d{1,2}|2[01]\d|22[0-3])(\.(\d|[1-9]\d|1\d{1,2}|2[0-4]\d|25[0-5])){2}\.(0|[1-9]\d?|1\d{1,2}|2[0-4]\d|25[0-5])$/");
        return regExp.test(item);
    }
    
    //ip sort
    sortService.sortIpStr = function(a, b) {
        if (!sortService.isIp(a)) {
            return 1;
        }
        
        if (!sortService.isIp(b)) {
            return -1;
        }
        ...
        while (i < arrA.length) {
            ...
            return sortService.sortNumberStr(arrA[i], arrB[i]);
        }
        return 0;
    }
    ...
    sortService.sortData = function(sortInfo, data /*datasource*/) {
        ...
        //排序操作结束后清除缓存 l10633 解决虚机状态排序后再主机状态排序使排序失效
...
var ngEventProvider = function (grid, $scope, domUtilityService, $timeout) {
    //fix width issue  --by w10450 2015-08-19
    var intervalTime = setInterval(function() {
        domUtilityService.RebuildGrid($scope,grid);
    }, 30);
    setTimeout(function() {clearInterval(intervalTime);}, 40);
    ...
    self.assignGridEventHandlers = function() {
        ...
        var windowResize = function(){
            ...
            windowThrottle = setTimeout(function() {
                ...
                domUtilityService.RebuildGrid($scope,grid);
                //add by l10191 打开任务台的时候（app.js）由于#main元素出现滚动条 导致nggrid获取的宽度不对
...
var ngFooter = function ($scope, grid) {
    ...
    $scope.pageForward = function() {
    	//修改问题单201707050585，CVM可以显示记录条数的界面没有记录时，鼠标放在“下一页”按钮上有一个禁止图标但实际可以翻页
    	if ($scope.cantPageForward()) {
    		return;
    	}
        var page = $scope.pagingOptions.currentPage;
        $scope.model.allSelected=false;//解决点击上一页下一页，全选框未去掉的问题
        //修改问题单201604161233 解决点击上一页下一页，去掉选中内容   -add by h10630 2016.4.27
...
var ngSearchProvider = function ($scope, grid, $filter, $utils) {
    ...
    $scope.filterText = '';
    //控制搜索框长度, 
    $scope.searchBoxSize = grid.config.searchBoxSize;
    ...
    var searchColumn = function(condition, item){
        ...
        if (typeof filter === "function") {
            ...
            for (var i = 0; i < primitiveValues.length; i++) {
            	result = condition.regex.test(primitiveValues[i]);
            }
...
//			}
    ...
    var filterFunc = function(item) {
        for (var x = 0, len = searchConditions.length; x < len; x++) {
            ...
            var result;
            if (!condition.column) {
                result = searchEntireRow(condition, item, self.fieldMap); //适配所有列进行查询
            } else {
                result = searchColumn(condition, item);//根据列名来查询
            }
            if(!result) {
                return false;
            }
    ...
    var buildSearchConditions = function (a) {
        ...
        if ($scope.columns.length > 0) {
        	qryField = $scope.columns[0].field;
        	if (qryField =="✔") {//若列表有checkbox，取第二列进行过滤查询
        		qryField = $scope.columns[1].field;
        	}
        	if (qryField == undefined) {
        		qryField = '';
        	}
        }
        ...
        for (var i = 0; i < columnFilters.length; i++) {
            ...
            if (args.length > 1) {
                ...
                if (val) {
                    searchConditions.push({
                        column: qryField,
                        columnDisplay: qryField,//修改全局过滤为只根据第一个列过滤
    ...
    $scope.$on('$destroy', $scope.$watch('filterText', function(newValue, oldValue){
    	if (newValue == oldValue) {
    	    return;
    	}
        if (!self.extFilter) {
            //$scope.$emit('ngGridEventFilter', newValue);
            buildSearchConditions(newValue);
            self.evalFilter();
        } else {
        	$scope.$emit('ngGridEventFilter', newValue);
        }
...
var ngSelectionProvider = function (grid, $scope, $parse, $utils) {
  ...
  // function to manage the selection action of a data item (entity)
    ...
    self.setSelection = function (rowItem, isSelected) {
        if(grid.config.enableRowSelection){
            if (!isSelected) {
                ...
                //修改问题单201603090075，解决列表内所有行复选框都选中时，取消一个后全选框未取消勾选的问题 by ckf6302
                ...
                if (self.getSelectionIndex(rowItem.entity) === -1) {
                  ...
                  //修改问题单201603090075，解决勾选最后剩余的一个行复选框时，全选框未同步勾选的问题  by ckf6302
                    if(grid.data.length==grid.config.selectedItems.length){
                    	$scope.model.allSelected=true;
                    }
                }
            }
    ...
    self.toggleSelectAll = function (checkAll, bypass, selectFiltered) {
    	$scope.model.allSelected=checkAll;
...
var ngStyleProvider = function($scope, grid) {
    ...
    $scope.rowStyle = function (row) {
    	//修改问题单201603100350，解决向左拖动列表内的列时水平分隔线不能顶到右侧边界的样式问题
//        var ret = { "top": row.offsetTop + "px", "height": $scope.rowHeight + "px" };	
    	var ret = { "top": row.offsetTop + "px", "height": $scope.rowHeight + "px","width":"100%" };
        if (row.isAggRow) {
    ...
    $scope.canvasStyle = function() {
    	//修改问题单201603100350，解决向左拖动列表内的列时水平分隔线不能顶到右侧边界的样式问题	by ckf6302
//        return { "height": grid.maxCanvasHt + "px" };
    	/**
    	 * desc: 解决部分列表(含有横向滚动条)中行分割线不能顶到右边界的问题
    	 * param: 
    	 * 		cvmVmListDivId	虚拟机列表div的id
    	 * 		workflowVmListDiv	云主机电子流列表div的id
    	 * 		previewUserListId	预览用户列表和预览虚拟机配置列表div的id（导入用户和导入虚拟机功能模块）
    	 */
    	var flag = $("."+grid.gridId).attr("id") == "cvmVmListDivId" || $("."+grid.gridId).attr("id") == "workflowVmListDiv" || 
    		$("."+grid.gridId).attr("id") == "previewUserListId";
    	if(flag){
    		var div = $("."+grid.gridId).find("div.ngCanvas")[0];
        	var scrollWidth=$("."+grid.gridId).find("div.ngCanvas")[0].scrollWidth;
        	var clientWidth=$("."+grid.gridId).find("div.ngViewport")[0].offsetWidth;
        	if (scrollWidth == clientWidth) {
        		//若直接返回值，虚拟机超过15个，会导致10 $digest() iterations reached. Aborting，
        		//为避开该问题，修改docment的style
    			div.style.width = "100%";
//    			return { "height": grid.maxCanvasHt + "px","width":"100%" };
    		} else {
    			div.style.width = "";
    		}
        	var maxCanvasHt = grid.maxCanvasHt; //若列表没有数据，默认也显示滚动条
        	if (maxCanvasHt == 0) {
        		maxCanvasHt = 1;
        	}
        	
    		return { "height": maxCanvasHt + "px" };
    	} else{							
        	return { "height": grid.maxCanvasHt + "px","width":"100%" };
    	}
//    	return { "height": grid.maxCanvasHt + "px" };
...
window.ngGrid.i18n['en'] = {
	    ...
	    ngTotalNumEnd :'',
	    ngMenuText: 'Columns:',
	    ngShowingItemsLabel: 'Current Items:',
	    ngTotalItemsLabel: 'Total Items:',
	    ngSelectedItemsLabel: 'Selected Items:',
	    ngPageSizeLabel: 'Items/p:',
	    ngPagerFirstTitle: 'First',
	    ngPagerNextTitle: 'Next',
	    ngPagerPrevTitle: 'Previous',
	    ngPagerLastTitle: 'Last'
	};
...
//    "<div class=\"ngSelectionCell\"><input tabindex=\"-1\" class=\"ngSelectionCheckbox\" type=\"checkbox\" ng-checked=\"row.selected\" /></div>"
   "<div class=\"ngSelectionCell\"><input tabindex='-1' class='ace ace-checkbox-2' type= 'checkbox' ng-checked='row.selected'  style='display:none;'/><span class='lbl sm'></span></div>"
  );
...
//    "<input class=\"ngSelectionHeader\" type=\"checkbox\" ng-show=\"multiSelect\" ng-model=\"allSelected\" ng-change=\"toggleSelectAll(allSelected, true)\"/>"
    "<div class=\"ngSelectionHeader\"><label ng-show=\"multiSelect\" style='margin:0'><input  class='ace ace-checkbox-2' type= 'checkbox'  ng-model=\"model.allSelected\" ng-change=\"toggleSelectAll(model.allSelected, true)\"  style='display:none;'/><span class='lbl sm'></span></label></div>"
  );
  ...
  $templateCache.put('footerTemplate.html',
    ...
    "\n" +
    "            <span class=\"ngLabel\" style='font-size:12px;line-height:25px'>{{i18n.ngTotalNum}}{{maxRows()}}{{i18n.ngTotalNumEnd}} {{i18n.ngNo}}{{pagingOptions.currentPage|num}}/{{maxPages()}}{{i18n.ngPage}}</span>\r" +
    "\n" +
    "        </div>\r" +
    "\n" +
...
//    "\n" +
    "    </div>\r" +
    "\n" +
    "    <div class=\"ngPagerContainer\" style=\"float: right; margin-top: 10px;font-size:12px;\" ng-show=\"enablePaging\" ng-class=\"{'ngNoMultiSelect': !multiSelect}\">\r" +
   
    "\n" +
    "        <div style=\"float:left; margin-right: 10px; line-height:25px;\" class=\"ngPagerControl\" style=\"float: left; min-width: 135px;\">\r" +
    "\n" +
    "            <div type=\"button\" ng-class=\"{'btn-forbidden':cantPageBackward()}\" class='ngPagerButton ' ng-click=\"pageToFirst()\" ng-disabled=\"cantPageBackward()\" title=\"{{i18n.ngPagerFirstTitle}}\"><span class='fa fa-angle-double-left ngPagerButtonSpan '></span></div>\r" +
    "\n" +
    "            <div type=\"button\" ng-class=\"{'btn-forbidden':cantPageBackward()}\" class='ngPagerButton ngPagerButton2' ng-click=\"pageBackward()\" ng-disabled=\"cantPageBackward()\" title=\"{{i18n.ngPagerPrevTitle}}\"><span class='fa fa-angle-left ngPagerButtonSpan '></span></div>\r" +
    "\n" +
    "            <div type=\"button\" ng-class=\"{'btn-forbidden':cantPageForward()}\" class='ngPagerButton ngPagerButton3'  ng-click=\"pageForward()\" ng-disabled=\"cantPageForward()\" title=\"{{i18n.ngPagerNextTitle}}\"><span class='fa fa-angle-right ngPagerButtonSpan '></span></div>\r" +
    "\n" +
    "            <div type=\"button\" ng-class=\"{'btn-forbidden':cantPageToLast()}\" class='ngPagerButton ngPagerButton2' ng-click=\"pageToLast()\" ng-disabled=\"cantPageToLast()\" title=\"{{i18n.ngPagerLastTitle}}\"><span class='fa fa-angle-double-right ngPagerButtonSpan '></span></div>\r" +
    "\n" +
    "        </div>\r" +
    "\n" +
    "        <div style=\"float:left; margin-right: 10px;\" class=\"ngRowCountPicker\">\r" +
    "\n" +
    "            <span style=\"float: left; margin-top: 0;line-height:25px\" class=\"ngLabel\">{{i18n.ngPageSizeLabel}}</span>\r" +
    "\n" +
    "            <select-input-nggrid options='pagingOptions.pageSizes' style=\"float: left;height: 25px; width: 50px;font-size:12px;border-color:#d5d5d5\" bind-model=\"pagingOptions.pageSize\" >\r" +
    "\n" +
//    "                <option ng-repeat=\"size in pagingOptions.pageSizes\">{{size}}</option>\r" +
    "\n" +
    "            </select-input-nggrid>\r" +
    "\n" +
    "        </div>\r" +
    "\n" +
    "    </div>\r" +
    "\n" +
    //在footer后面添加过滤框
    "    <div ng-show=\"showFilter\" style=\"float: right;margin-top:10px;margin-right:5px;\">\r" +
    "\n" +
    "        <input maxlength=\"64\" ng-style=\"{'width':searchBoxSize=='small'?'80px':'150px'}\" style=\"width:80px;height:25px;\" class=\"form-control\" placeholder=\"{{i18n.ngSearchPlaceHolder}}\" type=\"search\" ng-model=\"filterText\"/>\r" +
    "\n" +
    "    </div>\r" +
    "\n" +
  ...
  $templateCache.put('headerCellTemplate.html',
    ...
    "\n" +
    "\n    <div ng-click=\"col.sort($event)\" ng-class=\"'colt' + col.index\" class=\"ngHeaderText\">{{col.displayName}} \r" +
    "\n   <span ng-show=\"col.showSortButtonDown()\" class='icon-uparrow' ></span>" +
    "\n   <span ng-show=\"col.showSortButtonUp()\" class='icon-downarrow'></span></div>\r" +
...
//    "    </div>\r" +
    ...
    "\n" +           
    "                <label ng-hide='col.field == \"\u2714\"'><input  ng-disabled=\"col.pinned\" type=\"checkbox\" class=\"ace ace-checkbox-2 ngColListCheckbox\" ng-model=\"col.visible\"/ style='display:none'><span class='lbl sm colListCheckbox'></span>{{col.displayName}}</label>\r" +
    "\n" +
    "            </li>\r" +
    "\n" +
    "        </ul>\r" +
