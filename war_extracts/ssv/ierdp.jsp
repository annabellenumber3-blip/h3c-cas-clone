...
<%
    ...
    } else if ("1".equals(redirectClipboard)) {
        redirectClipboard = "TRUE";
    } else if ("0".equals(redirectClipboard)) {
    ...
    if (redirectCom == null) {
        redirectCom = "TRUE";
    } else if ("1".equals(redirectCom)) {
        redirectCom = "TRUE";
    } else if ("0".equals(redirectCom)) {
        redirectCom = "FALSE";
    }
	String redirectDrives = request.getParameter("enableDrive");
    if (redirectDrives == null) {
        redirectDrives = "TRUE";
    } else if ("1".equals(redirectDrives)) {
        redirectDrives = "TRUE";
    } else if ("0".equals(redirectDrives)) {
        redirectDrives = "FALSE";
    }
	String redirectPrinter = request.getParameter("enablePrinter");
    if (redirectPrinter == null) {
        redirectPrinter = "TRUE";
    } else if ("1".equals(redirectPrinter)) {
        redirectPrinter = "TRUE";
    } else if ("0".equals(redirectPrinter)) {
        redirectPrinter = "FALSE";
    }
   
	String redirectAudio = request.getParameter("enableAudio");
    if (redirectAudio == null) {
        redirectAudio = "0";
    } else if ("1".equals(redirectAudio)) {
        redirectAudio = "0";
    } else if ("0".equals(redirectAudio)) {
        redirectAudio = "2";
    }
	String redirectUsb = request.getParameter("enableUsb");
    if (redirectUsb == null) {
        redirectUsb = "TRUE";
    } else if ("1".equals(redirectUsb)) {
        redirectUsb = "TRUE";
    } else if ("0".equals(redirectUsb)) {
        redirectUsb = "FALSE";
    }
...
<body bgcolor="#ffffff" onload="windowFocus();BtnConnect();">
...
sub BtnConnect
   ...
   if Document.all.CheckBoxAutoLogon.checked then
     ...
     if Document.all.Domain.Value = "" then
      MsRdpClient.AdvancedSettings2.ClearTextPassword = ""
     else
	    MsRdpClient.AdvancedSettings2.ClearTextPassword = Document.all.Domain.Value
	   end if
   end if
   ...
   MsRdpClient.AdvancedSettings2.RedirectPrinters   = <%=redirectPrinter%>
   MsRdpClient.AdvancedSettings2.RedirectPorts      = <%=redirectCom%>
   MsRdpClient.AdvancedSettings2.RedirectSmartCards = TRUE
   MsRdpClient.AdvancedSettings2.RedirectClipboard = <%=redirectClipboard %>
 
   MsRdpClient.AdvancedSettings5.RedirectDevices = <%=redirectUsb%>

   MsRdpClient.SecuredSettings2.AudioRedirectionMode = <%=redirectAudio%>
...
<table border="0" width="100%" cellspacing="0" cellpadding=0 style="margin-top: -1em;">
<!-- Graphic bar row  -->
<!-- Row 1 -->
    <tr>
<!-- Column 1 spans 4 rows -->
	...
	<br>&nbsp;&nbsp;<input type="text" name="Server" size="41" id="editServer" value="<%=host%>">
...
<!-- Column 4 -->
<td align="bottom">
	 <p class=topspace>&nbsp;<input type="checkbox" name="CheckBoxAutoLogon" ID=Check1 checked="true" onclick = "checkClick"><label for="Check1" ID=SendLogonKey accesskey="l"><ID id=logoninfo>Send <u>l</u>ogon information for this connection&nbsp;</ID></label><br>
...
<span ID="tableLogonInfo" style="display: none">
			...
			<ID id=usernamelabel><u>U</u>ser&nbsp;name:</ID>
                <label id=UserNameKey accessKey="U" for="editUserName"><input type="text" name="UserName" id=editUserName size="25" value=""></label><br>
			<ID id=domainlabel><u>P</u>assword:</ID>
          <label id=editDomainKey accessKey="D" for="editDomain">
          <input type="password" name="Domain" id=editDomain size="25" value=""></label></p></span>
