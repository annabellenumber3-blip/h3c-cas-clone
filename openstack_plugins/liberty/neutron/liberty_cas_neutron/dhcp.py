...
class OVSDhcpPortMonitor(object):
    ...
    def _get_peer_name(self, prefix, name):
        if len(prefix + name) <= n_const.DEVICE_NAME_MAX_LEN:
            return prefix + name
        
        hashlen = 6
        namelen = n_const.DEVICE_NAME_MAX_LEN - len(prefix) - hashlen
        hashed_name = hashlib.sha1(name.encode('utf-8')) if isinstance(name,six.text_type) else hashlib.sha1(name)
        new_name = '%(prefix)s%(truncated)s%(hash)s' % {'prefix':prefix,'truncated':name[0:namelen],'hash':hashed_name.hexdigest()[0:hashlen]}
        LOG.warning(_("Creating an interface named %(name)s exceeds the %(limit)d character limitation. It was shortened to %(new_name)s to fit.")
                    % {'name':name,'limit':n_const.DEVICE_NAME_MAX_LEN,'new_name':new_name})
        return new_name
    ...
    def _setup_physical_bridges(self):
        ...
        for physical_network, bridge in six.iteritems(self._bridge_mappings):
            ...
            int_if_name = self._get_peer_name(constants.PEER_INTEGRATION_PREFIX,bridge)
            phys_if_name = self._get_peer_name(constants.PEER_PHYSICAL_PREFIX,bridge)
    ...
    def _restore_local_vlan_map(self):
        ...
        for port in cur_ports:
            ...
            if local_vlan:
                ...
                if (net_uuid and net_uuid not in self._local_vlan_hints and local_vlan != p_const.MAX_VLAN_TAG + 1):
    ...
    def _port_dead(self, port, log_errors=True):
        ...
        if cur_tag and cur_tag != p_const.MAX_VLAN_TAG + 1:
            self._int_br.set_db_attribute("Port",port.port_name,"tag",p_const.MAX_VLAN_TAG + 1,log_errors=log_errors)
    ...
    def _treat_vif_port(self, vif_port, port_id, network_id, network_type, physical_network,
        ...
        if vif_port:
            if admin_state_up:
                self._port_bound(vif_port,network_id,network_type,physical_network,
                                 segmentation_id,fixed_ips,device_owner,ovs_restarted)
            else:
    ...
    def _process_network_ports(self, port_info, ovs_restarted):
        ...
        if devices_added_updated:
            start = time.time()
            try:
                skipped_devices,need_binding_devices = self._treat_devices_added_or_updated(devices_added_updated,ovs_restarted)
                ...
                LOG.exception(_("process_network_ports - iteration:%d - failure while retrieving port details from server") % self.iter_num)
                resync_a = True
    ...
    def _treat_ancillary_devices_added(self, devices):
        ...
        LOG.info(_("Ancillary Ports %s added") % devices_added)
    ...
    def _treat_ancillary_devices_removed(self, devices):
        ...
        if failed_devices:
            LOG.debug(_("Port removal failed for %(devices)s") % failed_devices)
            resync = True
    ...
    def _process_ancillary_network_ports(self, port_info):
        ...
        if 'added' in port_info and port_info['added']:
            start = time.time()
            try:
                self._treat_ancillary_devices_added(port_info['added'])
                ...
                LOG.exception(_("process_ancillary_network_ports - iteration:%d - "
                                "failure while retrieving port details from server") % self.iter_num)
                resync_a = True
        if 'removed' in port_info and port_info['removed']:
            start = time.time()
            resync_b = self._treat_ancillary_devices_removed(port_info['removed'])
        ...
        while True:
            ...
            if sync:
                ...
                if consecutive_resyncs >= constants.MAX_DEVICE_RETRIES:
                    LOG.warn(_("Clearing cache of registered ports, retrials to resync were > %s") % constants.MAX_DEVICE_RETRIES)
                    ports.clear()
            ...
            if ovs_status == constants.OVS_DEAD:
                ...
                if polling_manager.is_polling_required or ovs_restarted:
                    try:
                        ...
                        if self._ancillary_brs:
                            ...
                            LOG.debug(_("Agent rpc_loop - iteration:%(iter_num)d - ancillary port info retrieved. "
                                        "Elapsed:%(elapsed).3f") % {'iter_num':self.iter_num,'elapsed':(time.time() - start)})
                        sync = False
                        if (port_info.get('added') or port_info.get('removed') or port_info.get('updated') or ovs_restarted):
                            ...
                            sync = self._process_network_ports(port_info,ovs_restarted)
                        ...
                        if self._ancillary_brs:
                            sync |= self._process_ancillary_network_ports(ancillary_port_info)
