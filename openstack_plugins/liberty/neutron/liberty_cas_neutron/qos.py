...
class QosCASAgentDriver(qos.QosAgentDriver):
    ...
    def _update_interface_qos(self, port, method, **configArgs):
        ...
        xmlstr = self._xml.encode_xml("config_qos",vmId=port['vmId'],mac=port['mac_address'],**configArgs)
    ...
    def update_bandwidth_limit(self, port, rule):
        ...
        try:
            ...
            if direction not in ['egress','ingress']:
                LOG.error(_("direction is neither egress or ingress"))
                raise cas_error.CasQosException()
    ...
    def delete_bandwidth_limit(self, port):
        self._update_interface_qos(port,'DELETE')
