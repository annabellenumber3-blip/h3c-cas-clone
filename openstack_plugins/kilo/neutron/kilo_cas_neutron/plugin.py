...
class CASNeutronAgent(db_base_plugin_v2.NeutronDbPluginV2,
    ...
    def __init__(self, agents):
        ...
        self._event = casevent.CasEvent(self._session,self._hostname,self)
    ...
    def _init_extension_manager(self):
        ...
        self.ext_manager.initialize(self.connection,'cas')
    ...
    def update_ports_status(self, instance_uuid, instance_id, ports, newStatus):
        for port_id in ports:
            ...
            try:
                ...
                changed = self._update_port_status(adminContext,instance_uuid,instance_id,port_id,vif_details,newStatus=newStatus)
                if changed:
    ...
    def _set_port_network_policy(self, context, port, vnic_type, vif_details):
        port_id = port['port_id']
        network_type = port['network_type']
        
        if self._neutron_plugin == 'openvswitch' and network_type == 'vlan':
        ...
        if 'vlanId' in vif_details:
            if vif_details['vlanId'] == segmentation_id:
                return False
    ...
    def _set_port_security_group(self, context, port, vif_details):
        port_id = port['port_id']
        newSecgroupIds = list()
    ...
    def _update_port_policies(self, context, instance_id, port_id, vnic_type, vif_details, ip_list):
        ...
        changed |= self._set_port_network_policy(context,port,vnic_type,vif_details)
        ...
        changed |= self._set_port_security_group(context,port,vif_details)
    ...
    def _get_secgroup_rules(self, context, secgroupId):
        ...
        if secgroupId == self.default_policy_name:
