...
def _patch_nova_api():
    ...
    srcDir = _get_purelib() + '/kilo_cas_patch/nova/api'
...
def _patch_nova_scheduler():
    ...
    srcDir = _get_purelib() + '/kilo_cas_patch/nova/scheduler'
...
def _patch_nova_conductor():
    ...
    srcDir = _get_purelib() + '/kilo_cas_patch/nova/conductor'
...
def _patch_nova_compute():
    ...
    srcDir = _get_purelib() + '/kilo_cas_patch/nova/compute'
...
def _patch_nova_objects():
    ...
    srcDir = _get_purelib() + '/kilo_cas_patch/nova/objects'
...
def _patch_ceilometer_agent():
    ...
    srcDir = _get_purelib() + '/kilo_cas_patch/ceilometer/agent'
    if os.path.exists(dstDir+'/base.py'):
        if os.path.exists(dstDir+'/original_base.py'):
            os.system("rm -f %s/base.py" % dstDir)
        else:
            os.system("mv %s/base.py %s/original_base.py" % (dstDir,dstDir))
        os.system("ln -s %s/base.py %s/base.py" % (srcDir,dstDir))

def _patch_ceilometer_compute():
    dstDir = _get_purelib() + '/ceilometer/compute'
    srcDir = _get_purelib() + '/kilo_cas_patch/ceilometer/compute'
...
def _post_install():
    ...
    os.system("echo \"echo 'kilo_cas_patch R0785P03'\" >> /usr/bin/openstack-cas-version")
