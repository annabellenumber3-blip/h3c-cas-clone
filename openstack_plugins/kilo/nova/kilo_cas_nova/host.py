...
class CasHost(object):
    ...
    def __init__(self, session, network, virtapi, virthost):
        ...
        self.port_update_pool = eventlet.GreenPool(size=CONF.cas.port_update_gthead_pool_size)
    ...
    def get_available_resources(self, nodename):
        baseInfo = {'hypervisor_type': 'kqemu',
                    'hypervisor_version': utils.convert_version_to_int('2.3'),
    ...
    def check_can_live_migrate_source(self, dest_check_data):
        ...
        dest_check_data['source_compute'] = host_info
        return dest_check_data
    
    def get_live_migrate_dest_host(self, instanceId, block_device_mapping, migrate_data, disks):
        source_compute = migrate_data['source_compute']
        if source_compute['hostIp'] != self._host_ip:
            LOG.error(_("Can't live migrate instance between different CVM."))
            raise cas_error.CasHostException()
        
        nodename = self.get_host_first_nodename()
        migrateArgs = {'vmId':instanceId,'migrateType':'live','clusterName':self._nodes[nodename]['clusterName']}
    ...
    def _sync_instances_node_by_migrate(self,instance,destHostId):
        try:
            srcNodeName = instance['node']
    ...
    def sync_instances_node_by_migrate(self,instance,destHostId):
        self.port_update_pool.spawn_n(self._sync_instances_node_by_migrate,instance,destHostId)
