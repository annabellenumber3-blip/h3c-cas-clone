...
class CasVMOps(object):
    ...
    def _get_vif_bandwidth_config(self, flavor):
        ...
        try:
            ...
            LOG.error(_("get bandwidth config from flavor(%s) failed.") % flavor)
            raise cas_error.CasVmException()
        ...
        if not exist:
            ...
            if instance_type == 'bare':
                ...
                if instance['image_ref'] and not recreate:
                    ...
                    sem.acquire()
                    try:
                        self._imageops.prepare_image_file(context,CONF.cas.glance_host,CONF.glance.port,imageMeta)
    ...
    def attach_interface(self, instance, image_meta, vif):
        ...
        self._network.attach_interface(vmInfo['id'],network_args['base_info'])
        self._network.update_vif_policy(vif,vmInfo['hostName'])
        
        bandwidth_conf = self._get_vif_bandwidth_config(instance.flavor)
        if bandwidth_conf:
            self._network.update_interface_qos(vmInfo['id'],vif,bandwidth_conf)
        
        if self._configVM:
            self._network.config_interface(vmInfo['id'],network_args['vif_info'])
    ...
    def get_cold_migrate_dest_node(self,instance,migration,sameFalvor):
        try:
            return self._host.get_cold_migrate_dest_node(migration.dest_node,instance['node'],sameFalvor)
        except Exception as exc:
