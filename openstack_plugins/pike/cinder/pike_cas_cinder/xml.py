...
class CasXml(object):
    ...
    def get_message_ids(self, root):
        msgIds=[]
        for taskmsg in root.findall("taskMsg"):
            msgId = taskmsg.find('msgId').text
            msgIds.append(msgId)
        return msgIds
...
class VolumeXml(CasXml):
    ...
    def update_volume_mode(self, **kwargs):
        root = etree.Element("volume")
        self.create_SubElement(root,**kwargs)
        return root
    ...
    def volume_get_info(self, root):
        ...
        poolType = root.find('poolType').text if root.find('poolType')!= None else None
        diskMode = root.find('diskMode').text if root.find('diskMode') != None else None
        mode = root.find('mode').text if root.find('mode') != None else None
        info = {'volume_format':volume_format,'writezero':writezero, 'mode': mode, 'size':size, 'poolType': poolType, 'diskMode': diskMode}
        
        return info
    
    def volume_qos_data_to_xml(self, **kwargs):
        root = etree.Element("volume")
        self.create_SubElement(root,**kwargs)
        return root
