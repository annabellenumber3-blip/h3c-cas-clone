...
def setup_instance_group(context, request_spec, *args, **kwargs):
    ...
    if any((policy in policies) for policy in group.policies):
        ...
        user_group_hosts = request_spec.instance_group.hosts
        user_hosts = set(user_group_hosts) if user_group_hosts else set()
        request_spec.instance_group.hosts = list(user_hosts|group_hosts)
        request_spec.instance_group.policie = group.policies
        request_spec.instance_group.members = group.members
    else:
        origin_utils.setup_instance_group(context,request_spec,*args,**kwargs)
