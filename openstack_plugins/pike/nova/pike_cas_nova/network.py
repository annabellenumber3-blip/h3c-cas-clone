...
class CasNetwork(object):
    ...
    def __init__(self, session, virthost,cvm_host):
        self._session = session
        self._virthost = virthost
        self._cvm_host = cvm_host
    ...
    def network_interface_info(self, network_info, net_model, net_queues, bindIp):
        ...
        for index in range(0,len(network_info)):
            ...
            if vif_info:
                ...
                if net_info['mtu']:
                    interface_info['mtu'] = net_info['mtu']
    ...
    def update_vif_name(self, vif, instanceUuid):
        self._update_vif_name(vif,instanceUuid)
    ...
    def update_vif_policy(self, vif, hostname):
        ...
        port_profile['cvm_host'] = self._cvm_host
    ...
    def update_vif_by_instance_migrate(self, instance_uuid, hostname):
        ...
        for port in vm_ports['ports']:
            ...
            profile_cvm_host = port_profile.get('cvm_host',None)
            if  profile_cvm_host is None or profile_cvm_host == self._cvm_host :
    ...
    def update_vif_by_instance_dts(self,instance_uuid,hostname,dest_nova_azone):
        _neutron_client = neutronapi.get_client(novacontext.get_admin_context())
        
        vm_ports = _neutron_client.list_ports(device_id=instance_uuid)
        LOG.info(_("begin to dts ports_update:uuid:%s,hostname:%s,vm_ports%s") % (instance_uuid,hostname,vm_ports))
        for port in vm_ports['ports']:
            port_profile = port.get('binding:profile',{})
            src_cvk_host = port_profile.get('cvk_host')
            ...
            LOG.info(_("port update check:vm:%s,%s,%s,%s,%s") %
                      (instance_uuid,host_id,src_cvk_host,hostname,profile_cvm_host))
            if profile_cvm_host is None or profile_cvm_host != self._cvm_host:
                port_profile['cvk_host'] = hostname
                port_profile['update_id'] = uuid.uuid1()
                port_profile['cvm_host'] = self._cvm_host
                ...
                try:
                    update_port = {'binding:profile':port_profile,'binding:host_id':self._virthost,'device_owner':'compute:%s'%dest_nova_azone}
                    LOG.info(_("begin to dts update profile:vm[%s],port[%s],info:%s")%(instance_uuid,port['id'],update_port))
                    _neutron_client.update_port(port['id'],{'port':update_port})
                    LOG.info(_("finish to dts update profile:vm[%s],port[%s],info:%s")%(instance_uuid,port['id'],update_port))
                except Exception as e:
                    LOG.info(_("failed to update profile:%s,%s,info:%s,exec:%s")%(instance_uuid,port['id'],update_port,e))
                    raise cas_error.CasNetworkException()
...
class CasNetworkOvs(CasNetwork):
    def _get_interface_vswitch(self, vif, choiceVswitch):
        ...
        if CONF.cas.soft_sdn == 'ovn':
            vswitch = CONF.cas.integration_bridge 
            if not vswitch:
                vswitch = 'br-int'
...
class CasNetworkVcfc(CasNetwork):
    ...
    def _get_interface_vswitch(self, vif, choiceVswitch):
        ...
        if vswitch == '*':
            if self._vswitches:
                ...
                LOG.error(_(
                    "The vswitch * can bind to physical network %s only when the option vswitches is set!") % physical_network)
                raise cas_error.CasNetworkException()
    ...
    def _update_vif_name(self, vif, instanceUuid):
        ...
        if vnic_type == 'normal':
            ...
            update_port ={'name':'vnet' + instanceUuid.replace('-','')[-5:] + vif['address'].lower().replace(':','')[-5:],
