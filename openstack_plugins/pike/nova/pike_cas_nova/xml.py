...
class HostXml(CasXml):
    ...
    def host_list(self, root):
        ...
        for host in root.findall("host"):
            ...
            if status != 1:
                continue
            haHostStatusObj = host.find("haHostStatus")
            if haHostStatusObj is not None:
                haHostStatus = int(haHostStatusObj.text)
                if haHostStatus == 1:
                    status = -1
            ...
            memIsolation = host.find("memIsolateStatus")
            if memIsolation is not None:
                memory_isolation = int(memIsolation.text)
            else:
                memory_isolation = 0
            ...
            hosts[name] = {"id":id,"name":name,"status":status,"cpu":cpu,"cpuRate":cpuRate,
                           ...
                           "local_raw_disks":local_raw_disks, "kernel_type":kernelType,
                           "memory_isolation": memory_isolation}
        
        return hosts

    def host_all_list(self, root):
        hosts = {}
        for host in root.findall("host"):
            id = host.find("id").text
            name = host.find("name").text
            ...
            if haHostStatusObj is not None:
                haHostStatus = int(haHostStatusObj.text)
                if haHostStatus == 1 and status != 0:
                    status = -1
    ...
    def numa_topology(self, root):
        ...
        for numa in root.findall("numa"):
            ...
            for cpu in numa.findall('cpu'):
                ...
                socket_obj = cpu.find('socket_id')
                if socket is None and socket_obj is not None and socket_obj.text is not None:
                    socket = int(socket_obj.text)
            ...
            numas.append({'id':int(id), 'socket': socket, 'cpus':set(cpus),'siblings':siblings,'memory':long(memory),'mempages':mempages})
        return numas
...
class VmXml(CasXml):
    ...
    def vm_protect_group(self, **kwargs):
        root = etree.Element("groupDomain")
        self.create_SubElement(root,**kwargs)
        return root
    ...
    def instance_modify_dev(self, **kwargs):
        root = etree.Element("domain")
        self.create_SubElement(root, **kwargs)
        return root
    ...
    def get_vlantrunk(self, root):
        ...
        return {'id': id, 'name': name, 'title': title, 'rule': rules}
