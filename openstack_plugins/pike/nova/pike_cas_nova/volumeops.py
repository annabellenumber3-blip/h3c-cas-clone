...
class CasVolumeOps(object):
    ...
    def get_root_volume_image(self, context, root_device_name, block_device_mapping):
        ...
        return self.get_volume_image_id(context,volume_id)

    def get_volume_image_id(self, context, volume_id):
    ...
    def query_volume_on_host(self, connection_info, hostName):
        ...
        elif volume_type in ['share','virtio','local']:
    ...
    def _get_local_volume_info(self, volume_data):
        storInfo = {'fileName': volume_data['volumeName'], 'size': volume_data['volumeSize'], 'format': volume_data['volume_format'], 'location': volume_data['location']}
        if volume_data.get('excludePrefix'):
            storInfo['excludePrefix'] = set(volume_data['excludePrefix'])
    ...
    def _get_volume_info(self, connection_info, mountpoint, disk_bus):
        volume_type = connection_info['driver_volume_type']
        serial_id = None
        if connection_info['serial']:
            serial_id = connection_info['serial'][0:20]
        volum_dict = {'deviceName':mountpoint,'targetBus':disk_bus,'serial': serial_id}
        ...
        if volume_type == 'rbd':
            ...
            volum_dict["type"] = "local"
            storInfo = self._get_local_volume_info(connection_info['data'])
            volum_dict.update(storInfo)
        else:
            LOG.error(_("volume type[%s] does not support!") % volume_type)
            raise cas_error.CasVolumeException()
        if connection_info.has_key('encryptionInfo') and connection_info['encryptionInfo']:
            volum_dict['encryptionInfo'] = connection_info['encryptionInfo']
        if qos_specs:
    ...
    def attach_lun_device_to_cluster(self, connection_info, hostName, nodename):
        volume_type = connection_info['driver_volume_type']
        if volume_type in ['share','virtio','local']:
            return
    ...
    def attach_volume(self, connection_info, instance, instanceId, mountpoint, disk_bus, tag=False):
        volume_info = self._get_volume_info(connection_info, mountpoint, disk_bus)
        if tag and connection_info.has_key('bootable') and connection_info['bootable']:
            volume_info['bootable'] = connection_info['bootable']
