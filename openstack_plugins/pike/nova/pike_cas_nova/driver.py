...
casapi_opts = [
    ...
    cfg.StrOpt('integration_bridge',
               default='br-int',
               help='Set the integration bridge for sdn mode to use. '
                    'Used only if we want to use ovn for neutron_plugin = openvswitch. '),
    cfg.StrOpt('hpName',
    ...
    cfg.IntOpt('glance_port',
               default=9292,
               help='the ip port of glance is used to operate image. '
                    'Used only if compute_driver is casapi.CasDriver. '),
    cfg.BoolOpt('runOnce',
                default=True,
    ...
    cfg.BoolOpt('castools_auto_upgrade',
                ...
                help='the Option to decide upgrade  vm castools automatically'
                     'Used only if compute_driver is casapi.CasDriver.'),
    cfg.BoolOpt('affinity_rules_map2cas',
                default=True,
                help='the Option to decide map openstack affinity rules to cas rules'
                     'Used only if compute_driver is casapi.CasDriver.'),
    cfg.IntOpt('event_gthead_pool_size',
               default=1000,
               help='the size of greethreads to dealwith port update,while migrating vm.'
               'Used only if compute_driver is casapi.CasDriver. '),
    cfg.BoolOpt('image_checksum',
                default=False,
                help='decide whether to caculate checksum whit images in cvm /vms/nova'
                     'Used only if compute_driver is casapi.CasDriver.'),
    cfg.BoolOpt('deploy_continuously_while_switchover',
                default=False,
                help='decide whether to keep vm deploying continuously while switchover'
               'Used only if compute_driver is casapi.CasDriver. '),
    cfg.IntOpt('dts_sync_pool_size',
                default=100,
                help='the pool size of greethreads for dts sync'
                'Used only if compute_driver is casapi.CasDriver. '),
    cfg.IntOpt('dts_sync_interval',
                default=-1,
                help='the interval of dts sync,unit:second.'
                     'dts_sync_state_interval > 0 :enable dts sync,such as:900'
                     'dts_sync_state_interval = -1:disable dts sync'
                     'Used only if compute_driver is casapi.CasDriver. '),
    cfg.BoolOpt('dts_sync_nofity',
                 default=False,
                 help='the Option to decide whether to notify the instance has dts sync occurred. '
                      'Used only if compute_driver is casapi.CasDriver. '),
    cfg.BoolOpt('dts_sync',
                 default=False,
                 help='the Option to decide whether to process dts event'
                     'Used only if compute_driver is casapi.CasDriver. '),
    cfg.StrOpt('event_type',
                ...
                help='the CAS event publish type,http/rabbitmq'
                     'Used only if compute_driver is casapi.CasDriver.'),
    cfg.StrOpt('rmq_host',
                help='ip addr for connection to CAS rabbitmq. Required if '
                     'compute_driver is casapi.CasDriver. '),
    cfg.IntOpt('rmq_port',
                default=5672,
    ...
    cfg.StrOpt('rmq_queue',
                default='cas_vm_event_nova_compute',
    ...
    cfg.StrOpt('rmq_queue_mode',
                ...
                help='the queue of connection to CAS rabbitmq,lazy/default'
                     'Used only if compute_driver is casapi.CasDriver.'),
    cfg.BoolOpt('wipe_disks',
                default=False,
                help='decide whether to wipe a volume before delete it'),
    cfg.StrOpt('os_plat',
                help='what openstack'),
    cfg.ListOpt('not_stay_state',
                default=['suspended', 'error', 'paused'],
                help='the capabilities of compute host will be report to scheduler. '),
]
...
keystone_authtoken_opts = [
    cfg.StrOpt('auth_uri',
        default='',
        help='see nova.conf,keystone_authtoken section'),
    cfg.StrOpt('auth_url',
        default='',
        help='see nova.conf,keystone_authtoken section'),
...
class CasDriver(driver.ComputeDriver):
    ...
    def init_host(self, host):
        ...
        if neutron_plugin == 'openvswitch':
            self._network = casnetwork.CasNetworkOvs(self._session,host,CONF.cas.host_ip)
        elif neutron_plugin == 'vcfc':
            self._network = casnetwork.CasNetworkVcfc(self._session,host,CONF.cas.host_ip)
        else:
        ...
        if CONF.cas.event_type =='http':
            self._event = casevent.CasEventHttp(self._session,self._host,host)
        elif CONF.cas.event_type =='rabbitmq':
            self._event = casevent.CasEventRmq(self._session,self._host,host)
    ...
    def cloud_get_physical_cpu(self):
        LOG.info(_("[cas::] begin to get total physical cpu."))
        physical_cpu = self._host.cloud_get_physical_cpu()
        LOG.info(_("[cas::] finish to get total physical cpu:%s")% physical_cpu)
        return physical_cpu
    ...
    def get_available_resource(self, nodename):
        ...
        LOG.info(_("[cas::] finish to get the available resource of resource:%s.") % resource)
    ...
    def change_instance_dev(self, context, instance, modifyArgs):
        LOG.info(_("[cas::] begin to change instance[%s] device.") % instance['uuid'])
        self._vmops.change_instance_dev(context, instance, modifyArgs)
        LOG.info(_("[cas::] finish to change instance[%s] device.") % instance['uuid'])
    ...
    def get_spice_console(self, context, instance):
        LOG.info(_("[cas::] begin to get instance[%s] spice console.") % instance['uuid'])
        spice_info = self._vmops.get_spice_console(instance)
        LOG.info(_("[cas::] finish to get instance[%s] spice console.") % instance['uuid'])
        return spice_info
    ...
    def destroy(self, context, instance, network_info, block_device_info=None,
                destroy_disks=True, migrate_data=None,wipe_disks=False):
        """Destroy VM instance."""
        ...
        if task_states.RESIZE_REVERTING == instance['task_state']:
            ...
            iswipe = instance['metadata'].get("wipe_disks",'False').lower() == 'true'
            wipe_volume = True if (iswipe or wipe_disks or CONF.cas.wipe_disks) else False
            self._vmops.destroy(context,instance,network_info,block_device_mapping,False,destroy_type,wipe_volume)
    ...
    def rebuild(self, context, instance, image_meta, injected_files,
        ...
        if nodeName and self._host.get_available_resources(nodeName) == -1:
            LOG.error(_("[cas::] The host [%s] is in maintenance mode or HA failure host.") % nodeName)
            raise Exception(_("The host [%s] is in maintenance mode or HA failure host.") % nodeName)
    ...
    def get_device_name_for_instance(self, instance, bdms, block_device_obj):
        ...
        if obj_device_name:
            ...
            if instance['image_ref']:
                if obj_disk_bus == sys_disk_bus and instance['root_device_name'][0:-1] == req_prefix:
                    if instance['root_device_name'][-1] == req_letter:
                        raise exception.InvalidDevicePath(path=obj_device_name)
            for bdm in bdms:
                device_name = bdm.device_name
                if (bdm.get('disk_bus') or sys_disk_bus) == obj_disk_bus and device_name[0:-1] == req_prefix:
    ...
    def extend_volume(self, connection_info, instance):
        """Extend volume of VM instance."""
        LOG.info(_("[cas::] begin to extend volume of instance[%s].") % instance['uuid'])
        LOG.info(_("[cas::] begin to extend volume of connection_info[ %s ].") % connection_info)
        self._vmops.extend_volume(connection_info, instance)
        LOG.info(_("[cas::] finish to extend volume of instance[%s].") % instance['uuid'])
    ...
    def get_host_ip_addr(self):
        ...
        new_instance_type = ArgInfo[-1]['new_instance_type']
        
        return self._vmops.get_cold_migrate_dest_node(instance,migration,new_instance_type)
    ...
    def live_migration(self, context, instance, dest,
        ...
        try:
            self._vmops.live_migration(instance,migrate_data)
        except Exception:
    ...
    def attach_cd(self, context, instance, cd_path):
        ...
        self._vmops.attach_cd(instance,cd_path)
    ...
    def detach_cd(self, context, instance, cd_id):
        ...
        self._vmops.detach_cd(instance,cd_id)
    ...
    def get_attached_cd(self, context, instance):
        ...
        cdrom_list = self._vmops.get_attached_cd(instance)
    ...
    def instance_set_boot_device(self, context, instance, boot_device):
        ...
        self._vmops.instance_set_boot_device(instance,boot_device)
    ...
    def cloud_unmanage_exist_vm(self,context,instance):
        ...
        self._vmops.cloud_affinity_rules_ummap2cas(instance)
        LOG.info(_("[cas::] finish to remove managed flag for instance[%s].") % instance['uuid'])
    
    def list_instances_uuids(self):
        LOG.info(_("[cas::] begin to get instances list uuids"))
        list_vm_uuids = self._host.list_instances_uuids()
        LOG.info(_("[cas::] finish to get instances list uuids"))
        return list_vm_uuids
    
    def get_instance_dts_info(self,context,instance):
        LOG.info(_("[cas::] begin to get vm dts info for instance[%s]") % instance['uuid'])
        dts_info = self._vmops.get_instance_dts_info(instance['uuid'])
        LOG.info(_("[cas::] finish to get vm dts info for instance[%s]") % instance['uuid'])
        return dts_info
    
    def pre_check_need_sync_dts_state(self,context,instance,local_dts_info):
        LOG.info(_("[cas::] begin to pre_check_need_sync_dts_state for instance[%s]") % instance['uuid'])
        need_sync = self._vmops.pre_check_need_sync_dts_state(instance,local_dts_info)
        LOG.info(_("[cas::] finish to pre_check_need_sync_dts_state for instance[%s]") % instance['uuid'])
        return need_sync
    
    def check_need_sync_dts_state(self,context,instance,local_dts_info,peer_dts_info):
        LOG.info(_("[cas::] begin to check_need_sync_dts_state for instance[%s]") % instance['uuid'])
        need_sync = self._vmops.check_need_sync_dts_state(instance,local_dts_info,peer_dts_info)
        LOG.info(_("[cas::] finish to check_need_sync_dts_state for instance[%s]") % instance['uuid'])
        return need_sync
    
    def sync_dts_state(self,context,instance,dts_info):
        LOG.info(_("[cas::] begin to sync dts vm state for instance[%s]") % instance['uuid'])
        self._vmops.sync_dts_state(instance,dts_info)
        LOG.info(_("[cas::] finish to sync dts vm state for instance[%s]") % instance['uuid'])
    ...
    def join_server_group(self, server_group, instance):
        """
        vm join to affinity group or anti-affinity group
        """
        LOG.info(_("[cas::] instance[%s] begin to add to affinity or anti-affinity group") % instance['uuid'])
        self._vmops.join_server_group(server_group, instance)
        LOG.info(_("[cas::] instance[%s] finish to add to affinity or anti-affinity group") % instance['uuid'])

    def remove_server_group(self, instance):
        """
        vm remove from affinity group or anti-affinity group
        """
        LOG.info(_("[cas::] instance[%s] begin to remove from affinity or anti-affinity group") % instance['uuid'])
        self._vmops.remove_server_group(instance)
        LOG.info(_("[cas::] instance[%s] finish to remove from affinity or anti-affinity group") % instance['uuid'])
...
class CasAPISession(object):
    ...
    def call_method(self, method, uri, **kwargs):
        ...
        try:
            ...
            if CONF.cas.deploy_continuously_while_switchover:
                self._check_session()
            raise excep
