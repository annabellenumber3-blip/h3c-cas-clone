...
def _post_install():
    ...
    srcDir = _get_purelib() + '/pike_cas_nova'
    ...
    if os.path.exists('/usr/bin/openstack-cas-version'):
        os.system("sed -i /pike_cas_nova/d /usr/bin/openstack-cas-version")
    os.system("echo \"echo 'pike_cas_nova R0785P03'\" >> /usr/bin/openstack-cas-version")
    os.system("chmod a+x /usr/bin/openstack-cas-version")
    os.system("cp -r pike_cas_nova/*.pem %s" % dstDir)
