...
def _post_install():
    ...
    trunk_dstDir = _get_purelib() + '/neutron/services/trunk/drivers/cas'
    srcDir = _get_purelib() + '/pike_cas_neutron'
    trunk_srcDir = _get_purelib() + '/pike_cas_neutron/trunk'

    os.system("rm -rf %s" % dstDir)
    os.system("rm -rf %s" % trunk_dstDir)
    os.system("ln -s %s %s" % (srcDir,dstDir))
    os.system("ln -s %s %s" % (trunk_srcDir, trunk_dstDir))
    ...
    if os.path.exists('/usr/bin/openstack-cas-version'):
        os.system("sed -i /pike_cas_neutron/d /usr/bin/openstack-cas-version")
    os.system("echo \"echo 'pike_cas_neutron R0785P03'\" >> /usr/bin/openstack-cas-version")
    os.system("chmod a+x /usr/bin/openstack-cas-version")
    os.system("cp -r pike_cas_neutron/*.pem %s" % dstDir)
