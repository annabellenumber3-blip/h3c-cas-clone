...
class CASNeutronAclSecurity(CASNeutronSecurity):
    ...
    def prepare_security_group(self, secgroupId, rules):
        if secgroupId == self.default_accept_policy_name:
            security_group = {'name':'openstack-'+secgroupId,
                              'defaultAclAction':self.acl_action['YES'],
                              'defaultAclOutAction':self.acl_action['YES'],
                              'rule':rules}
        else:
            security_group = {'name':'openstack-'+secgroupId,
        ...
        if rule['protocol'] == 'ICMP':
            ...
            if rule['fromPort'] == 1 and rule['toPort'] == 65535:
                ...
                if direction == 0:
                    ...
                    if rule['fromPort'] != rule['toPort']:
                        if CONF.cas.acl_in_port == 'src':
                            acl_rule['srcPort'] = rule['fromPort']
                            acl_rule['srcPortEnd'] = rule['toPort']
                        else:
                            acl_rule['destPort'] = rule['fromPort']
                            acl_rule['destPortEnd'] = rule['toPort']
                    else:
    ...
    def check_security_group_rules(self, secgroupId, rules):
        ...
        if resp.status_code == 200:
            ...
            if secgroupId == self.default_accept_policy_name:
                if (acl_rules['defaultAclAction'] != self.acl_action['YES']
                    or acl_rules['defaultAclOutAction'] != self.acl_action['YES']):
                    return False
            else:
