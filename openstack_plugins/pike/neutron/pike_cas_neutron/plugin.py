...
from neutron.services.trunk.drivers.cas.agent import driver as trunk_driver
...
class CASNeutronAgent(db_base_plugin_v2.NeutronDbPluginV2,
    ...
    def __init__(self, agents):
        ...
        self._trunk_driver = trunk_driver.CasTrunkDriver(cas_plugin=self)
    ...
    def _setup_rpc(self):
        ...
        self.cache_plugin_rpc = agent_rpc.CacheBackedPluginApi(topics.PLUGIN)
    ...
    def _init_extension_manager(self):
        ...
        self.ext_manager = ext_manager.L2AgentExtensionsManager(self.conf)
    ...
    def update_subport_port_down(self, context, port_id):
        try:
            self.plugin_rpc.update_device_down(context, port_id, self._agent_id, self._hostname)
        except Exception:
            LOG.error(_("fail to update status down for port_id:%s") % port_id)

    def update_subport_port_up(self, context, port_id):
        try:
            self.plugin_rpc.update_device_up(context, port_id, self._agent_id, self._hostname)
        except Exception:
            LOG.error(_("fail to update status up for port_id:%s") % port_id)
    ...
    def update_ports_status(self, instance_uuid, instance_id, ports, newStatus, check=True):
        for port_id in ports:
            if check and port_id not in self._ports:
                continue
    ...
    def _get_instance_network_objs(self, vm_id):
        uri = self._session.make_cmd_uri('/vm/network',vm_id)
        resp, body = self._session.call_method("GET",uri)
        if resp.status_code == 200:
            network_objs = self._xml.decode_xml("instance_network_objs",body)
            return network_objs
        else:
            LOG.error(_("fail to get instance[vm_id=%s] network! uri:%s") % (vm_id, uri))
            LOG.error(_("fail to get instance[vm_id=%s] network! resp.status_code:%d,%s")
                       % (vm_id,resp.status_code,cas_error.get_http_respond_error(resp)))
            raise cas_error.CasVmException()

    def get_instance_network_mtu(self, vm_id, mac):
        network_objs = self._get_instance_network_objs(vm_id)
    ...
    def _set_port_network_policy(self, context, port, vnic_type, vif_details, tags):
        ...
        if vnic_type == portbindings.VNIC_DIRECT:
            ...
            if CONF.cas.config_mtu:
                ...
                if mtu and int(mtu) != int(self.get_instance_network_mtu(port['vmId'], port['mac_address'])):
                    policy_args['mtu'] = mtu
                    mtu_change = True
        ...
        if not vlan_change and not tag_change and not mtu_change:
            return False
    ...
    def _set_port_security_group(self, context, port, vif_details, tags):
        ...
        if pvlan_id:
            ...
            newSecgroupIds = port.get('security_groups',[]) or self.get_port(context,port_id)['security_groups']
    ...
    def _get_port_network_info(self,context,port,port_tags):
        vif_info = {}
        fixed_ip_list = []
        bootproto_list = []
        for fixed_ip in port['fixed_ips']:
            ...
            if subnet['enable_dhcp']:
                ...
                if port_tags.get('enable_gateway') == 'enable_gateway' and subnet['gateway_ip']:
                    ip_info['gateway'] = subnet['gateway_ip']
                elif port_tags.get('disable_gateway') == 'disable_gateway':
                    pass
                else:
    ...
    def _set_port_network_config(self,context,instance_id,port,vif_details,ip_list,force,tag_update):
        ...
        def _get_tags_info(context,port_id):
            ...
            try:
                ...
                LOG.info(_("success to get port:%s tags:%s") % (port_id,tags_list))
                if tags_list:
                    for tag_info in tags_list:
                        LOG.info(_("tag info is :%s" ) % tag_info)
                        if tag_info.startswith('enable_gateway'):
                            port_tags['enable_gateway'] = tag_info
                        if tag_info.startswith('disable_gateway'):
                            port_tags['disable_gateway'] = tag_info
            except Exception:
                LOG.warning(_("fail to get tags of port:%s.") % port_id)
            return port_tags
        port_tags = _get_tags_info(context, port['port_id'])
        vif_info,fixed_ip_list,bootproto_list = self._get_port_network_info(context,port,port_tags)
        fixed_ips_str = ','.join(fixed_ip_list)
        vip_list = []
        port_fixedip_list = []
        if (port_tags.get("enable_gateway") == "enable_gateway" or port_tags.get("disable_gateway") == "disable_gateway") and force is False and tag_update is True:
            update_gateway = True
        if vif_info:
        ...
        if 'fixed_ips' not in vif_details:
            ...
            LOG.info(_("update_gateway is %s" % update_gateway))
            if vif_details['fixed_ips'] == fixed_ips_str and not update_gateway:
                is_fixdips_changed = False
            elif vif_details['fixed_ips'] != fixed_ips_str or update_gateway:
        ...
        if 'allowed_ips' not in vif_details:
            ...
            if vif_details['allowed_ips'] != allowed_ips_str or (update_gateway and vip_list):
                is_allowedips_changed = self._config_allowed_ips(instance_id, port_network_info)
            else:
                is_allowedips_changed = False
    ...
    def _update_port_policies(self, context, instance_id, port_id, vnic_type, vif_details, ip_list, force=False, tag_update=False):
        def _get_tags_info(context,port_id):
            port_tags = {}
            try:
                tags_list = self.tag_plugin.get_tags(context,'ports',port_id)['tags']
        ...
        devices_details_list = self.cache_plugin_rpc.get_devices_details_list(context, [port_id], self._agent_id)
        port = devices_details_list[0]
        if not port.has_key('network_type'):
        ...
        if vnic_type == portbindings.VNIC_DIRECT:
            ...
            return changed
        if CONF.cas.soft_sdn != 'ovn':
            changed |= self._set_port_qos_policy(context,port,vif_details)
        if CONF.cas.configPort:
            changed |= self._set_port_network_config(context,instance_id,port,vif_details,ip_list,force,tag_update)
        
        if self._neutron_plugin == 'vcfc' or network_type == 'vxlan' or CONF.cas.soft_sdn == 'ovn':
    ...
    def _update_port(self, port, sync_status,tag_update):
        port_id = port['id']
        ...
        try:
            ...
            try:
                ...
                try:
                    changed |= self._update_port_policies(adminContext,vm_id,port_id,binding_port['vnic_type'],vif_details,ip_list,force=sync_status,tag_update=tag_update)
                except Exception as exc:
                ...
                # trunk检测
                t1 = Thread(target=self._trunk_driver.check_trunk, args=(adminContext, port))
                t1.start()
                # self._trunk_driver.check_trunk(adminContext, port)
            finally:
                sem.release()
                self._semObj.back_obj_sem(port_id)
    ...
    def port_update(self, context, **kwargs):
        ...
        if self._hostname != port.get(portbindings.HOST_ID):
            ...
            self._update_port(port,False,True)
    ...
    def network_update(self, context, **kwargs):
        ...
        network_info = self.get_network(context, network_id)
        description = network_info.get('description')
        if description:
            rule = eval(description)
            if 'action' in rule and 'name' in rule and 'vmVlan' in rule and 'vswitchVlan' in rule:
                action = rule['action']
                class Trunk:
                    def __init__(self, id):
                        self.id = id

                vlan_trunk = self._trunk_driver.get_vlantrunk(Trunk(rule['name']))
                if vlan_trunk and action == 'Add':
                    vlan_trunk['rule'].append({'vmVlan': rule['vmVlan'], 'vswitchVlan': rule['vswitchVlan']})
                elif vlan_trunk and action == 'Remove':
                    del_rule = None
                    for cas_rule in vlan_trunk['rule']:
                        if cas_rule['vmVlan'] == rule['vmVlan']:
                            del_rule = cas_rule
                            break
                    vlan_trunk['rule'].remove(del_rule)
                self._trunk_driver.update_vlantrunk(vlan_trunk)
        ...
        for port_id in self._ports.keys():
            if self._ports[port_id] == network_id:
                ...
                self._update_port(port,False,False)
    ...
    def _get_secgroup_rules(self, context, secgroupId):
        ...
        for rule in secgroup_rules:
            ...
            if rule['remote_secgroup']:
                ...
                if rule['remote'] not in cache_port_sg_bindings:
                    ports = self._get_port_security_group_bindings(context, filters)
                    cache_port_sg_bindings[rule['remote']] = ports
                else:
                    ports = cache_port_sg_bindings[rule['remote']]
                for port in ports:
                    try:
                        devices_details_list = self.cache_plugin_rpc.get_devices_details_list(context,[port['port_id']],self._agent_id)
                        port_info = devices_details_list[0]
                        if 'network_type' not in port_info:
                            port_info = self.get_port(context,port['port_id'])
                    except exceptions.PortNotFound:
                        continue
                    for fixed_ip in port_info['fixed_ips']:
                        sunet_uuid =  fixed_ip['subnet_id']
                        if sunet_uuid not in cache_subnet_info:
                            subnet = self.get_subnet(context,sunet_uuid)
                            ip_version = subnet['ip_version']
                            cache_subnet_info[sunet_uuid] = ip_version
                        else:
                            ip_version = cache_subnet_info[sunet_uuid]
                        
                        if rule['ethertype'] != 'IPv%d' % ip_version:
                            continue
                        if rule['ethertype'] == 'IPv4':
    ...
    def _update_remote_security_groups(self, context, secgroupIds):
        update_remote_secgroups = set()
        
        filters = {'remote_group_id': secgroupIds}
        rules = self.get_security_group_rules(context,filters)
        for rule in rules:
            update_remote_secgroups.add(rule['security_group_id'])
        
        for secgroup in update_remote_secgroups:
            try:
    ...
    def _sync_ports_info(self, context):
        if not CONF.cas.enable_sync_port_info:
            return
        binding_ports = []
        ...
        for binding_port in self._get_binding_ports(context):
            ...
            self._update_port(port,True, False)
