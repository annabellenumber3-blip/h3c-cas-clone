...
from neutron.agent.casagent import error as exceptions
...
class CasClient:
    ...
    def http_log_resp(self, req_uuid, resp, stream):
        if not self._http_log_debug:
            return
        
        if stream:
            LOG.debug("[cas response: %s]\nRESP: [%s] %s\n" % (req_uuid,resp.status_code,resp.headers))
        else:
    ...
    def request(self, method, uri, **kwargs):
        ...
        self.http_log_resp(req_uuid,resp,stream)
        
        self.auth_chal = auth.chal

        if not stream and resp.text:
