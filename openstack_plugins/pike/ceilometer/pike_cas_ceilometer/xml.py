...
class InspectorXml(CasXml):
    ...
    def instance_diagnostic_connections(self, root):
        connections = root.find('flowLink')
        if connections is not None:
            return long(connections.text)
    ...
    def instance_diagnostic_all(self, root):
        ...
        connections = self.instance_diagnostic_connections(root)
        gpusInfo = self.instance_diagnostic_gpu(root)
        
        return {'basicInfo':basicInfo,'domMemStat':domMemStat,'domBlkStat':domBlkStat,
                'domIfStat':domIfStat,'domUsage':domUsage,'domBlkRate':domBlkRate,
                'domIfRate':domIfRate,'connections':connections, 'gpusInfo': gpusInfo}
    ...
    def instance_diagnostic_gpu(self, root):
        gpusInfo = []

        for gpuRate in root.findall('domgpustat'):
            symbol = gpuRate.find('symbol').text
            type = gpuRate.find('type').text
            utilization = float(gpuRate.find('utilization').text)
            graphicMemoryUse = float(gpuRate.find('graphicMemoryUse').text)
            encodeRate = float(gpuRate.find('encodeRate').text)
            decodeRate = float(gpuRate.find('decodeRate').text)
            gpusInfo.append({'symbol': symbol,
                             'type': type,
                             'utilization': utilization,
                             'graphicMemoryUse': graphicMemoryUse,
