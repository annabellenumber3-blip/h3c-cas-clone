...
def _patch_nova_api():
    ...
    srcDir = _get_purelib() + '/mitaka_cas_patch/nova/api'
...
def _patch_nova_scheduler():
    ...
    srcDir = _get_purelib() + '/mitaka_cas_patch/nova/scheduler'
...
def _patch_nova_conductor():
    ...
    srcDir = _get_purelib() + '/mitaka_cas_patch/nova/conductor'
...
def _patch_nova_compute():
    ...
    srcDir = _get_purelib() + '/mitaka_cas_patch/nova/compute'
...
def _patch_nova_objects():
    ...
    srcDir = _get_purelib() + '/mitaka_cas_patch/nova/objects'
...
def _patch_ceilometer_agent():
    ...
    srcDir = _get_purelib() + '/mitaka_cas_patch/ceilometer/agent'
    if os.path.exists(dstDir+'/manager.py'):
        if os.path.exists(dstDir+'/origin_manager.py'):
            os.system("rm -f %s/manager.py" % dstDir)
        else:
...
def _patch_ceilometer_compute():
    ...
    srcDir = _get_purelib() + '/mitaka_cas_patch/ceilometer/compute'
...
def _post_install():
    ...
    if os.path.exists('/usr/bin/openstack-cas-version'):
        os.system("sed -i /mitaka_cas_patch/d /usr/bin/openstack-cas-version")
    os.system("echo \"echo 'mitaka_cas_patch R0785P03'\" >> /usr/bin/openstack-cas-version")
