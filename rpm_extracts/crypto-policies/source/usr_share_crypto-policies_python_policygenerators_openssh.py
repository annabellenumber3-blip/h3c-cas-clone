...
class OpenSSHClientGenerator(OpenSSHGenerator):
	...
	def test_config(cls, config):
		...
		if ret and config.find('sk-') != -1:
			cls.eprint("Ignoring the OpenSSH check result with sk- algos in config")
			ret = False
		if ret:
...
class OpenSSHServerGenerator(OpenSSHGenerator):
	...
	def generate_config(cls, policy):
		...
		return cls.generate_options(policy, local_kx_map, local_gss_kx_map, True)
