...
typedef enum {
    ...
    VIR_CONNECT_LIST_STORAGE_POOLS_FC            = 1 << 30,
} virConnectListAllStoragePoolsFlags;
...
typedef enum {
    ...
    VIR_STORAGE_VOL_CREATE_PREALLOC_FALLOC = 1 << 6,
    VIR_STORAGE_VOL_CREATE_PREALLOC_FULL = 1 << 7,

    /* added by caizhifeng on 20131031 for 201308160117 */
    VIR_STORAGE_VOL_CREATE_INC_IMAGE_BY_CP = 1 << 8,
...
typedef enum {
    VIR_STORAGE_POOL_LOCK       = 1 << 0, /* lock acquire */
    VIR_STORAGE_POOL_UNLOCK     = 1 << 1, /* unlock acquire */
} virStoragePoolLockFlags;

int virStoragePoolLockAcquire(virStoragePoolPtr pool,
                              unsigned int flags);

int virStorageVolCreateXMLBrief(virStoragePoolPtr pool,
                                const char *xmldesc,
                                unsigned int flags);

int virStorageVolDeleteBrief(virStoragePoolPtr pool,
                             const char *vol_name,
                             unsigned int flags);

int virStoragePoolListAllVolumesBrief(virStoragePoolPtr pool,
                                      virStorageVolPtr **vols,
                                      unsigned int flags);

int virConnectLockFreeAllStoragePools(virConnectPtr conn,
                                      unsigned int flags);
...
typedef enum {
    ...
    VIR_STORAGE_POOL_EVENT_SHUTDOWN = 6, /* for cas pool shutdown */
...
typedef enum {
    VIR_STORAGE_POOL_STOP_NORMAL = 0,
    VIR_STORAGE_POOL_STOP_REFRESH = 1,

# ifdef VIR_ENUM_SENTINELS
    VIR_STORAGE_POOL_STOP_LAST
# endif
} virStoragePoolEventStopReason;

typedef enum {
    VIR_STORAGE_POOL_SHUTDOWN_EXCEPT = 0,

# ifdef VIR_ENUM_SENTINELS
    VIR_STORAGE_POOL_SHUTDOWN_LAST
# endif
} virStoragePoolEventShutDownReason;
