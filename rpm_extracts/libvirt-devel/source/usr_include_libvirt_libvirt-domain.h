...
typedef enum {
    ...
    VIR_DOMAIN_RUNNING_COLO = 11,           /* restored from colo migration */
...
typedef enum {
    ...
    VIR_DOMAIN_PAUSED_UPGRADE = 14,     /* gaochengbo add*/
    VIR_DOMAIN_PAUSED_COLO = 15,        /* pause due to colo migration */
...
typedef enum {
    ...
    /* Migration with all host devices is allowed */
    VIR_MIGRATE_ALLOWED_HOSTDEV   = (1 << 18),
    
    /*
     * Virtual machine Shared memory migration
     * */
    VIR_MIGRATE_ZEROCOPY          = (1 << 19),
    
    /*
     * normal memory to sharedmemory migrate
     * */
    VIR_MIGRATE_SHAREMEMORY       = (1 << 20),
    
   /*
    *sharememory to normal memory migrate
    * */     
    VIR_MIGRATE_NORMALMEMORY      = (1 << 21),
...
/**
 * VIR_MIGRATE_PARAM_DEST_IFNAME:
 *
 * virDomainUpgrade* params field: the name to be used for the domain on the
 * destination host as VIR_TYPED_PARAM_STRING. Omitting this parameter keeps
 * the domain name the same. This field is only allowed to be used with
 * hypervisors that support domain renaming during migration.
 */
# define VIR_MIGRATE_PARAM_DEST_IFNAME         "destination_ifname" 
...
/* Domain Upgrade. */
virDomainPtr virDomainUpgrade (virDomainPtr domain,
                               virConnectPtr dconn,
                               virTypedParameterPtr params,
...
typedef enum {
    ...
    VIR_DOMAIN_XML_AUTOSTART_RESULT = (1 << 4), /* dump XML autostart result */
} virDomainXMLFlags;
...
int                     virDomainInterfaceStats (virDomainPtr dom,
                                                 ...
                                                 size_t size);
int                     virConnectDomainLiveUpdate(virDomainPtr domain,
                                                 const char *domainXml);
int                     virDomainSetVcacheStatus(virDomainPtr domain,
                                                 unsigned int status);
int                     virDomainLiveUpdateReconnect(virDomainPtr domain,
                                                     unsigned int pid);
...
typedef enum {
    ...
    /* Add for disk serial id */
    VIR_DOMAIN_DEVICE_MODIFY_DISK_SERIAL_ID = (1 << 3),
...
int virConnectMdevCheck(virConnectPtr conn,
                        const char *uuid, unsigned int flags);
...
typedef enum {
    VIR_DOMAIN_BLOCK_JOB_INFO_BANDWIDTH_BYTES = 1 << 0, /* bandwidth in bytes/s
                                                           instead of MiB/s */
    VIR_DOMAIN_BLOCK_JOB_INFO_PERSIST         = 1 << 1, /* persist flag, not
                                                           return false even
                                                           failed in checking
                                                           commit process*/
...
typedef enum {
    VIR_DOMAIN_BLOCK_JOB_INFO_SUCCESS = 0,
    VIR_DOMAIN_BLOCK_JOB_INFO_FAILED_TO_BEGIN_QEMU_JOB_MODIFY = 1,
} virDomainBlockJobInfoFailedReason;
...
struct _virDomainBlockJobInfo {
    ...
    virDomainBlockJobInfoFailedReason fr;
};
...
typedef enum {
    ...
    VIR_DOMAIN_EVENT_STARTED_NOTIFY_HA = 5, /* started notify HA */
...
typedef enum {
    ...
    VIR_DOMAIN_EVENT_SUSPENDED_COLO = 9, /* suspended due to colo migration */
...
typedef enum {
    ...
    VIR_DOMAIN_EVENT_RESUMED_COLO = 4,       /* Resumed from colo migration */
...
typedef enum {
    ...
    VIR_DOMAIN_EVENT_STOPPED_NOTIFY_HA = 7, /* stopped notify HA */
...
typedef enum {
    ...
    VIR_DOMAIN_JOB_OPERATION_UPGRADE_IN = 10,
    VIR_DOMAIN_JOB_OPERATION_UPGRADE_OUT = 11,
    VIR_DOMAIN_JOB_OPERATION_COLO = 12,
...
typedef enum {
    ...
    VIR_CONNECT_DOMAIN_EVENT_AGENT_LIFECYCLE_REASON_QGA_UPGRADE = 3, /* recevied qga upgrade command */
    VIR_CONNECT_DOMAIN_EVENT_AGENT_LIFECYCLE_REASON_QUERY_MEMORY = 4, /* recevied qga query memory command */
    VIR_CONNECT_DOMAIN_EVENT_AGENT_LIFECYCLE_REASON_QUERY_CPU = 5, /* recevied qga query cpu command */
...
int virDomainExtraneousInformation(virDomainPtr dom,
                                   int flagnum,
                                   virTypedParameterPtr params,
                                   int *nparams);
/**
 * VIR_DOMAIN_DYNAMIC_SET_MEMORY_NUM:
 *
 * Macro represents the number of dynamic setting memory, as a uint.
 */
#define VIR_DOMAIN_DYNAMIC_SET_MEMORY_NUM "cyclenumber"

/**
 * VIR_DOMAIN_RESERVED_MEMORY:
 *
 * Macro represents the reserved memory for domain, as a uint.
 */
#define VIR_DOMAIN_RESERVED_MEMORY  "reservedmemory"
...
typedef enum {
    VIR_DOMAIN_COLO_START   = 1 << 2,  /* start colo guest */
    VIR_DOMAIN_COLO_FAILOVER = 1 << 3, /* failover colo guest */
    VIR_DOMAIN_COLO_RESUME = 1 << 4,   /* resume colo guest */
} virDomainColoFlags;

int
virDomainColo(virDomainPtr domain, const char *xml, unsigned int flags);

int virDomainSetCgroupMemSwappiness(virDomainPtr domain,
                                    unsigned int swappiness,
                                    unsigned int flags);
int
virDomainSetUcacheMode(virDomainPtr domain,
                         unsigned int mode);
