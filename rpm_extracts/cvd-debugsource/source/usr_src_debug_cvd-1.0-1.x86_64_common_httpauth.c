...
{
	static char const hex[18] = "0123456789abcdef";
	unsigned i;
...
int httpauth_set_user_pwd(httpauth_t *auth, char *username, char *password)
{
	...
	cvd_strncpy(auth->username, username, sizeof(auth->username));
	cvd_strncpy(auth->password, password, sizeof(auth->password));
...
{
	if(strlen(realm) > REALMLEN || strlen(nonce) > REALMLEN || strlen(cnonce) > REALMLEN)
		return 1;

	cvd_strncpy(auth->realm, realm, sizeof(auth->realm));
	cvd_strncpy(auth->nonce, nonce, sizeof(auth->nonce));
    cvd_strncpy(auth->cnonce, cnonce, sizeof(auth->cnonce));
...
int httpauth_get_response(httpauth_t *auth, char *cmd, char *url, char *qop, char *response)
{
    ...
    if ('\0' == qop[0]) {
        memset(strH1, 0, 512);
    	sprintf((char*)strH1, "%s:%s:%s", md5_h1, auth->nonce, md5_h2);
    	len = strlen((char*)strH1);
    	md5(strH1, len, result);
    	to_hex((char*)result, 16, (unsigned char*)response);
    } else {
        memset(strH1, 0, 1024);
    	sprintf((char*)strH1, "%s:%s:%s:%s:%s:%s", md5_h1, auth->nonce, "00000001", auth->cnonce, qop, md5_h2);
    	len = strlen((char*)strH1);
    	md5(strH1, len, result);
    	to_hex((char*)result, 16, (unsigned char*)response);
    }
