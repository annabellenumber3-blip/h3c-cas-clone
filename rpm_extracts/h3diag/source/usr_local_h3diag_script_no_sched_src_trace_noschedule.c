...
static int __init trace_noschedule_init(void)
{
	...
	struct noschedule_info *info = &nosched_info;
#if LINUX_VERSION_CODE < KERNEL_VERSION(3, 15, 0)
        struct tracepoint_iter *iter = kmalloc(sizeof(struct tracepoint_iter), GFP_KERNEL);
        struct tracepoint *tp = *(iter->tracepoint);
#endif
	stack_trace_skip_hardirq_init();
        ...
        tp = *(iter->tracepoint);
        if (tp) 
		tracepoint_lookup(tp, info);
        else
                printk("ERROR:NO first tracepoint.\n");

        while (info->tp_initalized < info->tp_needinit)
        {
